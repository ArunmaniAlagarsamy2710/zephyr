...
static int siwx91x_configure_feature_bitmap(const struct device *dev,
					    siwx91x_feature_bitmap_cfg_t cfg, uint8_t val)
{
	struct siwx91x_dev *sidev = dev->data;
	...
	case HIDDEN_SSID:
		...
		if (sidev->hidden_ssid && !val) {
			sidev->hidden_ssid = false;
			sidev->reboot_needed = true;
		} else if (!sidev->hidden_ssid && val > 0) {
			sidev->hidden_ssid = true;
			sidev->reboot_needed = true;
		}

		break;
	case AP_MAX_STA:
		if (sidev->max_num_sta != val) {
			sidev->max_num_sta = val;
			sidev->reboot_needed = true;
		}
...
static int siwx91x_ap_enable(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	enum wifi_ap_status event_status;
	...
	if (FIELD_GET(SIWX91X_INTERFACE_MASK, interface) != SL_WIFI_AP_INTERFACE) {
		...
		event_status = WIFI_STATUS_AP_OP_NOT_PERMITTED;
		ret = -EINVAL;
		goto raise_events;
	}
	...
	if (params->band != WIFI_FREQ_BAND_UNKNOWN && params->band != WIFI_FREQ_BAND_2_4_GHZ) {
		...
		event_status = WIFI_STATUS_AP_OP_NOT_SUPPORTED;
		ret = -ENOTSUP;
		goto raise_events;
	}
	...
	if (params->ssid_length == 0 || params->ssid_length > WIFI_SSID_MAX_LEN) {
		...
		event_status = WIFI_STATUS_AP_SSID_NOT_ALLOWED;
		ret = -EINVAL;
		goto raise_events;
	}
	...
	if (sec < 0) {
		...
		event_status = WIFI_STATUS_AP_AUTH_TYPE_NOT_SUPPORTED;
		ret = -EINVAL;
		goto raise_events;
	}
	...
	if (ret < 0) {
		event_status = WIFI_STATUS_AP_FAIL;
		goto raise_events;
	}
...
raise_events:
	wifi_mgmt_raise_ap_enable_result_event(sidev->iface, event_status);
	return ret;
}
