...
} siwx91x_wlan_module_evt_state_t;
...
typedef enum {
	...
	WLAN_REASON_MAX
} siwx91x_wlan_evt_reason_code_t;

static const char *siwx91x_get_reason_string(uint8_t reason_code)
{
	switch (reason_code) {
	case WLAN_REASON_NO_REASON:
		return "No reason specified";
	case WLAN_REASON_AUTH_DENIAL:
		return "Authentication denial";
	case WLAN_REASON_ASSOC_DENIAL:
		return "Association denial";
	case WLAN_REASON_AP_NOT_PRESENT:
		return "AP not present";
	case WLAN_REASON_UNKNOWN:
		return "Unknown";
	case WLAN_REASON_HANDSHAKE_FAILURE:
		return "Four way handshake failure";
	case WLAN_REASON_USER_DEAUTH:
		return "Deauthentication from User";
	case WLAN_REASON_PSK_NOT_CONFIGURED:
		return "PSK not configured";
	case WLAN_REASON_AP_INITIATED_DEAUTH:
		return "De-authentication (AP induced Roam/Deauth from supplicant)";
	case WLAN_REASON_ROAMING_DISABLED:
		return "Roaming not enabled";
	default:
		return "UNKNOWN";
	}
}

static int siwx91x_notify_connect_event(struct net_if *iface, enum wifi_conn_status event_status,
					int ret)
{
...
static int siwx91x_notify_ap_event(struct net_if *iface, enum wifi_ap_status event_status, int ret)
{
							   ...
							   uint32_t result_length, void *arg)
{
	...
	case STATE_ASSOCIATED:
		if (FIELD_GET(SIWX91X_INTERFACE_MASK, interface) == SL_WIFI_AP_INTERFACE) {
			siwx91x_notify_ap_event(sidev->iface, WIFI_STATUS_AP_SUCCESS, 0);
	...
	case STATE_UNASSOCIATED:
		if (FIELD_GET(SIWX91X_INTERFACE_MASK, interface) == SL_WIFI_CLIENT_INTERFACE) {
			...
			/*FIXME its not triggering while disable the ap */
...
static bool siwx91x_is_param_changed(struct wifi_iface_status *prev_params,
                                     struct wifi_connect_req_params *new_params)
{
	...
	if (memcmp(new_params->ssid, prev_params->ssid, prev_params->ssid_len) != 0) {
		return true;
	}

	if (new_params->security != prev_params->security) {
		return true;
	}

	if (new_params->channel != WIFI_CHANNEL_ANY &&
...
static int siwx91x_validate_ap_reconfig(const struct device *dev,
				        struct wifi_connect_req_params *new_params)
{
	/*FIXME need to discuss the max inactivity time, need global var to store the previous val */
	struct wifi_iface_status prev_params = { 0 };
	...
	if (sidev->reboot_needed || siwx91x_is_param_changed(&prev_params, new_params)) {
		return siwx91x_ap_disable(dev);
	}

	ret = sl_net_get_credential(SL_NET_DEFAULT_WIFI_AP_CREDENTIAL_ID, &psk_type, prev_psk,
				    &prev_psk_length);
	if (ret < 0) {
...
static int siwx91x_validate_sta_reconfig(const struct device *dev,
					struct wifi_connect_req_params *new_params)
{
	struct wifi_iface_status prev_params = { 0 };
	uint32_t prev_psk_length = WIFI_PSK_MAX_LEN;
	...
	if (siwx91x_is_param_changed(&prev_params, new_params)) {
		return siwx91x_disconnect(dev);
	}

	ret = sl_net_get_credential(SL_NET_DEFAULT_WIFI_CLIENT_CREDENTIAL_ID, &psk_type, prev_psk,
				    &prev_psk_length);
	if (ret < 0) {
		LOG_ERR("Failed to get credentials: 0x%x", ret);
		return -EIO;
...
static int siwx91x_ap_enable(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	if (sidev->state == WIFI_STATE_COMPLETED) {
		ret = siwx91x_validate_ap_reconfig(dev, params);
		if (ret < 0) {
			return ret;
		}
	}
	...
	if (sec < 0) {
		...
		return siwx91x_notify_ap_event(sidev->iface, WIFI_STATUS_AP_AUTH_TYPE_NOT_SUPPORTED,
					       -EINVAL);
	}
	...
	if (ret != SL_STATUS_OK) {
		...
		return siwx91x_notify_ap_event(sidev->iface, WIFI_STATUS_AP_FAIL, -EINVAL);
	}
	...
	if (ret != SL_STATUS_OK) {
		...
		return siwx91x_notify_ap_event(sidev->iface, WIFI_STATUS_AP_FAIL, -EIO);
	}
...
static int siwx91x_connect(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	if (sidev->state == WIFI_STATE_COMPLETED) {
		ret = siwx91x_validate_sta_reconfig(dev, params);
		if (ret < 0) {
			return ret;
		}
	}
	...
	/*FIXME Do we need to handle the case if ap is not available during reconnection? */
...
static int siwx91x_ap_config_params(const struct device *dev, struct wifi_ap_config_params *params)
{
	...
	ret = sl_wifi_get_ap_configuration(SL_WIFI_AP_INTERFACE, &siwx91x_ap_cfg);
	if (ret < 0) {
		LOG_ERR("Failed to get ap configuration: 0x%x", ret);
		return -EIO;
	}
