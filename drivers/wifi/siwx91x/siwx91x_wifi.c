...
static int siwx91x_handle_ps_param_wakeup_mode(struct siwx91x_dev *sidev,
...
static int siwx91x_handle_ps_param_timeout(sl_wifi_performance_profile_t *sl_ps_profile,
					   struct wifi_ps_params *params)
{
...
static int siwx91x_handle_ps_param_exit_strategy(struct siwx91x_dev *sidev,
						 struct wifi_ps_params *params,
						 sl_wifi_performance_profile_t *sl_ps_profile)
{
...
static int siwx91x_set_power_save(const struct device *dev, struct wifi_ps_params *params)
{
	...
	case WIFI_PS_PARAM_WAKEUP_MODE:
		status = siwx91x_handle_ps_param_wakeup_mode(sidev, params, sl_ps_profile);
		if (status < 0) {
			return status;
		}
	...
	case WIFI_PS_PARAM_TIMEOUT:
		status = siwx91x_handle_ps_param_timeout(sl_ps_profile, params);
		if (status < 0) {
			return status;
		}
	...
	case WIFI_PS_PARAM_EXIT_STRATEGY:
		status = siwx91x_handle_ps_param_exit_strategy(sidev, params, sl_ps_profile);
		if (status <= 0) {
			return status;
		}
